...
export default function FactureEditForm({ id }: { id: string }) {
  const router = useRouter();
  const [form, setForm] = useState<Partial<Facture> | null>(null);
  ...
  return (
    <div className="bg-white p-8 rounded-xl shadow-xl w-full max-w-[1600px] mx-auto mt-8">
      <h1 className="text-3xl font-bold text-gray-800 mb-8">Modifier la facture</h1>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
              ...
              <input type="text" name="numero" value={form.numero || ""} onChange={handleChange} className="w-full border p-3 rounded" placeholder="Numéro de facture" />
            </div>
            <div>
