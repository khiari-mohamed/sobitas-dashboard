...
const Devis = ({ order, printRef }: { order: Record<string, unknown>; printRef?: React.Ref<HTMLDivElement> }) => {
      ...
      <div ref={printRef} className="bg-white p-8 rounded-xl shadow-xl w-full max-w-2xl mx-auto print:shadow-none print:border-none print:rounded-none print:p-0">
        ...
        <div className="flex justify-between items-start mb-4">
          ...
          <div className="flex flex-col items-end">
            ...
            <span className="text-xs text-gray-500">N° : <span className="font-semibold">{String(order.numero_devis || order.numero)}</span></span>
            <span className="text-xs text-gray-500">Validité : {String(order.validite_devis || "30 jours")}</span>
          </div>
        </div>
