...
const initialState: Partial<Review> = {
  id: "",
  user_id: "",
  product_id: "",
  stars: "5",
...
export default function ReviewsCreateClient() {
  const router = useRouter();
  const [form, setForm] = useState<Partial<Review>>(initialState);
  ...
  return (
    <div className="bg-white p-8 shadow-xl w-full max-w-[1600px] mx-auto mt-8 border">
      <h1 className="text-3xl font-bold text-gray-800 mb-8">Ajouter un nouvel avis</h1>
      <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-8">
        <div className="mb-6">
          <label className="block text-xl font-semibold mb-2">ID</label>
          <input type="text" name="id" value={form.id || ""} onChange={handleChange} className="w-full border p-4 text-base" placeholder="Auto-généré" />
        </div>
        <div className="mb-6">
