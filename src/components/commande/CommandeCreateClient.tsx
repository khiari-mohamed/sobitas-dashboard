...
const initialState: Partial<Commande> = {
  nom: "",
  prenom: "",
  email: "",
  phone: "",
...
export default function CommandeCreateClient() {
  const router = useRouter();
  const [form, setForm] = useState<Partial<Commande>>(initialState);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const numero = form.numero && form.numero.trim() !== "" ? form.numero : `CMD-${Date.now()}`;
  ...
  return (
    <div className="bg-white p-8 rounded-xl shadow-xl w-full max-w-[1600px] mx-auto mt-8">
      <h1 className="text-3xl font-bold text-gray-800 mb-8">Créer une nouvelle commande</h1>
